...
**Response**: the message option in the load\_data code chunk displays
what the output of the console is in the rmd file.
...
**Response**:To answer this question we would only need Major and
unemployment\_rate variables. I would the sort the dataset by the lowest
unemployment\_rate.
...
**Response**: lowest\_unemploy &lt;- select(college\_recent\_grads,rank,
major, unemployment\_rate)
...
``` r
highest_prop_women <- arrange(college_recent_grads, desc(sharewomen)) %>% 
...
**Response**:Early Childhood education is the major with the highest
proportion of woman.
