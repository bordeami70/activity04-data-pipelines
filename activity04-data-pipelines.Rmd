...
college_recent_grads <-read_csv(file ="data/recent-grads.csv")
...
**Response**: the message option in the load_data code chunk displays what the output of the console is in the rmd file.
...
**Response**:To answer this question we would only need Major and unemployment_rate variables. I would the sort the dataset by the lowest unemployment_rate.
...
**Response**: lowest_unemploy <- select(college_recent_grads,rank, major, unemployment_rate)
...
**Response**:Early Childhood education is the major with the highest proportion of woman.
...
```{r}
stem_salaries <- college_recent_grads %>% 
  select(major,median)
